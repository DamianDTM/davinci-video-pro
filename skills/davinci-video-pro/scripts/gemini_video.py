...
def analyze(client: genai.Client, args: argparse.Namespace) -> None:
    ...
    prompt = ('Analiza este archivo audiovisual para una edicion cuya FUENTE PRINCIPAL es el guion del usuario. '
    ...
    try:
        remote = client.files.upload(file=source, config=types.UploadFileConfig(mime_type=mime, display_name=source.name))
        ...
        response = client.interactions.create(
            model=model, store=False, system_instruction=SYSTEM_PROMPT,
            input=[media_input, {'type': 'text', 'text': prompt}],
            response_format={'type': 'text', 'mime_type': 'application/json',
                             'schema_': ANALYSIS_SCHEMA},
        ...
        if not isinstance(result, dict) or not isinstance(result.get('escenas'), list):
            raise RuntimeError('La respuesta no cumple la estructura de analisis esperada.')
        missing = [name for name in ANALYSIS_SCHEMA['required'] if name not in result]
        if missing:
            raise RuntimeError('El informe esta incompleto: '+', '.join(missing))
...
def main() -> int:
    ...
    media = sub.add_parser('analyze', help='Sube un archivo autorizado y guarda un informe JSON.')
    ...
    media.add_argument('--processing', choices=('agentic', 'static'), default='agentic')
